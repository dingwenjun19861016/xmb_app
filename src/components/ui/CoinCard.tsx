...
export interface CoinCardData {
  ...
  coin24h?: Array<{    // 24小时价格变化数据（兼容旧字段名）
    price: number;
    createdAt: string;
  }>;
  stock24h?: Array<{   // 24小时股票价格变化数据（新字段名）
    price: number;
    createdAt: string;
  }>;
}
    ...
    <View style={[
      ...
      <View style={styles.bottomRow}>
        ...
        <View style={styles.chartAndChangeContainer}>
          {/* 24小时价格图表 */}
          {(data.stock24h || data.coin24h) && (data.stock24h || data.coin24h).length > 1 && (
            <View style={styles.chartContainer}>
              <SVGMiniPriceChart
                data={data.stock24h || data.coin24h}
